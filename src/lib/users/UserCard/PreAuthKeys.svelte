<script lang="ts">
	import { getPreauthKeys, expirePreAuthKey, deletePreAuthKey } from '$lib/common/apiFunctions.svelte';
	...
	function deletePreAuthKeyAction(preAuthKeyID: string) {
		deletePreAuthKey(preAuthKeyID)
			.then(() => {
				getPreauthKeysAction();
			})
			.catch((error) => {
				...
				{#each keyList as key}
						...
						<tr class:hidden={$preAuthHideStore && ((key.used && !key.reusable) || new Date(key.expiration).getTime() < new Date().getTime())}>
							...
							<td>
								...
								<!-- trash symbol -->
								<button class="mr-2" on:click={() => {expirePreAuthKeyAction(key.id)}}
								...
								{/if}
								<!-- delete button -->
								<button class="mr-2 text-error" on:click={() => {deletePreAuthKeyAction(key.id)}}
									><svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 inline flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
										<path stroke-linecap="round" stroke-linejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
									</svg></button
								>
